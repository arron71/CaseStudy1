...
## A list of the merged dataset columns and how many NA's are contained in each

## Brew_ID
length(which(is.na(breweryAndBeerData$Brew_ID)))

## BreweryName
length(which(is.na(breweryAndBeerData$BreweryName)))

## City
length(which(is.na(breweryAndBeerData$City)))

## State
length(which(is.na(breweryAndBeerData$State)))

## BeerName
length(which(is.na(breweryAndBeerData$BeerName)))

## Beer_ID
length(which(is.na(breweryAndBeerData$Beer_ID)))

## ABV
length(which(is.na(breweryAndBeerData$ABV)))

## IBU
length(which(is.na(breweryAndBeerData$IBU)))

## Style
length(which(is.na(breweryAndBeerData$Style)))

## Ounces
length(which(is.na(breweryAndBeerData$Ounces)))
```
...
```
  #The ABV has a mean of 0.5977 and median of 0.067.  This would indicate this data has a skewed distribution.  Should more advanced statistics be run, the tests should be robust due to the large sample size in accordance to the Central Limit Theorem.
...
BandBplot <- ggplot(breweryAndBeerData2, aes(x = ABV, y = IBU)) + geom_point(size = 2, shape = 23) + geom_smooth(method= "lm") + labs(title = "Relationship between ABV and IBU", x = "ABV", y = "IBU") + theme(plot.title = element_text(hjust = 0.5))
BandBplot
```
